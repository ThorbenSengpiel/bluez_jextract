// Generated by jextract

package bluez.binding;

import java.lang.invoke.MethodHandle;
import java.lang.invoke.VarHandle;
import java.nio.ByteOrder;
import jdk.incubator.foreign.*;
import static jdk.incubator.foreign.ValueLayout.*;
public class _GHashTableIter {

    static final  GroupLayout $struct$LAYOUT = MemoryLayout.structLayout(
        Constants$root.C_POINTER$LAYOUT.withName("dummy1"),
        Constants$root.C_POINTER$LAYOUT.withName("dummy2"),
        Constants$root.C_POINTER$LAYOUT.withName("dummy3"),
        Constants$root.C_INT$LAYOUT.withName("dummy4"),
        Constants$root.C_INT$LAYOUT.withName("dummy5"),
        Constants$root.C_POINTER$LAYOUT.withName("dummy6")
    ).withName("_GHashTableIter");
    public static MemoryLayout $LAYOUT() {
        return _GHashTableIter.$struct$LAYOUT;
    }
    static final VarHandle dummy1$VH = $struct$LAYOUT.varHandle(PathElement.groupElement("dummy1"));
    public static VarHandle dummy1$VH() {
        return _GHashTableIter.dummy1$VH;
    }
    public static MemoryAddress dummy1$get(MemorySegment seg) {
        return (MemoryAddress)_GHashTableIter.dummy1$VH.get(seg);
    }
    public static void dummy1$set( MemorySegment seg, MemoryAddress x) {
        _GHashTableIter.dummy1$VH.set(seg, x);
    }
    public static MemoryAddress dummy1$get(MemorySegment seg, long index) {
        return (MemoryAddress)_GHashTableIter.dummy1$VH.get(seg.asSlice(index*sizeof()));
    }
    public static void dummy1$set(MemorySegment seg, long index, MemoryAddress x) {
        _GHashTableIter.dummy1$VH.set(seg.asSlice(index*sizeof()), x);
    }
    static final VarHandle dummy2$VH = $struct$LAYOUT.varHandle(PathElement.groupElement("dummy2"));
    public static VarHandle dummy2$VH() {
        return _GHashTableIter.dummy2$VH;
    }
    public static MemoryAddress dummy2$get(MemorySegment seg) {
        return (MemoryAddress)_GHashTableIter.dummy2$VH.get(seg);
    }
    public static void dummy2$set( MemorySegment seg, MemoryAddress x) {
        _GHashTableIter.dummy2$VH.set(seg, x);
    }
    public static MemoryAddress dummy2$get(MemorySegment seg, long index) {
        return (MemoryAddress)_GHashTableIter.dummy2$VH.get(seg.asSlice(index*sizeof()));
    }
    public static void dummy2$set(MemorySegment seg, long index, MemoryAddress x) {
        _GHashTableIter.dummy2$VH.set(seg.asSlice(index*sizeof()), x);
    }
    static final VarHandle dummy3$VH = $struct$LAYOUT.varHandle(PathElement.groupElement("dummy3"));
    public static VarHandle dummy3$VH() {
        return _GHashTableIter.dummy3$VH;
    }
    public static MemoryAddress dummy3$get(MemorySegment seg) {
        return (MemoryAddress)_GHashTableIter.dummy3$VH.get(seg);
    }
    public static void dummy3$set( MemorySegment seg, MemoryAddress x) {
        _GHashTableIter.dummy3$VH.set(seg, x);
    }
    public static MemoryAddress dummy3$get(MemorySegment seg, long index) {
        return (MemoryAddress)_GHashTableIter.dummy3$VH.get(seg.asSlice(index*sizeof()));
    }
    public static void dummy3$set(MemorySegment seg, long index, MemoryAddress x) {
        _GHashTableIter.dummy3$VH.set(seg.asSlice(index*sizeof()), x);
    }
    static final VarHandle dummy4$VH = $struct$LAYOUT.varHandle(PathElement.groupElement("dummy4"));
    public static VarHandle dummy4$VH() {
        return _GHashTableIter.dummy4$VH;
    }
    public static int dummy4$get(MemorySegment seg) {
        return (int)_GHashTableIter.dummy4$VH.get(seg);
    }
    public static void dummy4$set( MemorySegment seg, int x) {
        _GHashTableIter.dummy4$VH.set(seg, x);
    }
    public static int dummy4$get(MemorySegment seg, long index) {
        return (int)_GHashTableIter.dummy4$VH.get(seg.asSlice(index*sizeof()));
    }
    public static void dummy4$set(MemorySegment seg, long index, int x) {
        _GHashTableIter.dummy4$VH.set(seg.asSlice(index*sizeof()), x);
    }
    static final VarHandle dummy5$VH = $struct$LAYOUT.varHandle(PathElement.groupElement("dummy5"));
    public static VarHandle dummy5$VH() {
        return _GHashTableIter.dummy5$VH;
    }
    public static int dummy5$get(MemorySegment seg) {
        return (int)_GHashTableIter.dummy5$VH.get(seg);
    }
    public static void dummy5$set( MemorySegment seg, int x) {
        _GHashTableIter.dummy5$VH.set(seg, x);
    }
    public static int dummy5$get(MemorySegment seg, long index) {
        return (int)_GHashTableIter.dummy5$VH.get(seg.asSlice(index*sizeof()));
    }
    public static void dummy5$set(MemorySegment seg, long index, int x) {
        _GHashTableIter.dummy5$VH.set(seg.asSlice(index*sizeof()), x);
    }
    static final VarHandle dummy6$VH = $struct$LAYOUT.varHandle(PathElement.groupElement("dummy6"));
    public static VarHandle dummy6$VH() {
        return _GHashTableIter.dummy6$VH;
    }
    public static MemoryAddress dummy6$get(MemorySegment seg) {
        return (MemoryAddress)_GHashTableIter.dummy6$VH.get(seg);
    }
    public static void dummy6$set( MemorySegment seg, MemoryAddress x) {
        _GHashTableIter.dummy6$VH.set(seg, x);
    }
    public static MemoryAddress dummy6$get(MemorySegment seg, long index) {
        return (MemoryAddress)_GHashTableIter.dummy6$VH.get(seg.asSlice(index*sizeof()));
    }
    public static void dummy6$set(MemorySegment seg, long index, MemoryAddress x) {
        _GHashTableIter.dummy6$VH.set(seg.asSlice(index*sizeof()), x);
    }
    public static long sizeof() { return $LAYOUT().byteSize(); }
    public static MemorySegment allocate(SegmentAllocator allocator) { return allocator.allocate($LAYOUT()); }
    public static MemorySegment allocateArray(int len, SegmentAllocator allocator) {
        return allocator.allocate(MemoryLayout.sequenceLayout(len, $LAYOUT()));
    }
    public static MemorySegment allocate(ResourceScope scope) { return allocate(SegmentAllocator.nativeAllocator(scope)); }
    public static MemorySegment allocateArray(int len, ResourceScope scope) {
        return allocateArray(len, SegmentAllocator.nativeAllocator(scope));
    }
    public static MemorySegment ofAddress(MemoryAddress addr, ResourceScope scope) { return RuntimeHelper.asArray(addr, $LAYOUT(), 1, scope); }
}


